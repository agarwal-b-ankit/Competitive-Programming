...
ll head[MAXN],heavy[MAXN],heavyw[MAXN],pos[MAXN],endpos[MAXN],tim=0;
...
void hld(ll u, ll h, ll wt){
    head[u]=h;
    a[tim]=wt;
    pos[u]=tim++;
    if(heavy[u]!=-1)
        hld(heavy[u],h,heavyw[u]);
    for(auto p:adj[u]){
        ll v=p.ff,w=p.ss;
...
ll query_hld(ll u, ll v, ll left, ll right){
    ...
    ll cur=query(1,0,tim-1,pos[u]+1,pos[v],left,right);
    ans+=cur;
    return ans;
}

// currently queries edge weighted scenarios
// can be easily modified to handle vertex weighted scenarios
int main()
{
	FastIO
	ll n,q,i,u,v,l;
	...
	for(i=0;i<q;i++){
	    cin>>u>>v>>l;
	    // queries the no of edges with value between 0 and l in the path of u to v
	    cout<<query_hld(u,v,0,l)<<"\n";
	}
	return 0;
}
